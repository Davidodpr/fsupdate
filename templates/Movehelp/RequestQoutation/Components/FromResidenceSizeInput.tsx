...
import { buttonWrapperVariants, questionIconWrapperVariants, popupWrapperVariants, inputWrapperVariants } from '../RequestQuotation.variants'
...
export const FromResidenceSizeInput = ({
  ...
  const [localSqm, setLocalSqm] = React.useState(getValues('sqm') || '')
  const [localAdditionalSpace, setLocalAdditionalSpace] = React.useState(getValues('additionalSpace') || '')

  const prevValuesRef = React.useRef({ sqm: null as number | null, additionalSpace: null as string | null })
  ...
  const handleSqmChange = (value: string) => {
    ...
    if (setData) {
      const sqmValue = value ? Number(value) : null
      const updatedData: MoveValues = {
        sqm: sqmValue,
        additionalSpace: localAdditionalSpace || null,
      ...
      if (setInternalSqm && sqmValue) {
        setInternalSqm(sqmValue)
      }
    }
  }
          ...
          <Flex direction="row" justifyContent="space-between" alignItems="center">
            ...
            <div className={buttonWrapperVariants()}>
              <Button
                type="submit"
                withFullWidth
                disabled={localSqm === '' || !Number(localSqm)}
