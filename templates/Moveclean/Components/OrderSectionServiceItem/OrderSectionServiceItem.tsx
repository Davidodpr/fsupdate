...
const OrderSectionServiceItem = ({ price, popupText, service, fromAddress, toAddress, residenceSize, date, additionalSpace, setDatePicked }: ServiceItemProps) => {
  ...
  return (
    <div className={clsx(itemVariants({ noMarginBottom: isAlwaysIncluded }))}>
      <div className={clsx(topSectionVariants({ paddingBottom: true }))}>
        ...
        {!isAlwaysIncluded && (
          <Flex direction={'row'} justifyContent="center" alignItems="end">
            ...
            <Popup withFluidWidth text={<div className={clsx(textWrapperVariants())}>{popupText}</div>}>
              <QuestionIcon style={{ marginBottom: 2 }} color="var(--fs-colors-questionIconColor)" alt="question icon" width={24} height={24} />
            </Popup>
          </Flex>
        )}
      </div>
      <div className={clsx(textSectionWrapperVariants())}>
        {isService && (
          <div className={clsx(penWrapperVariants())} onClick={() => setShowInputs(!showInputs)}>
            <PencilSimple />
          </div>
        )}
        {fromAddress && (
        ...
        {residenceSize && (
          <>
            {showInputs ? (
              <>
                <Text className={clsx(boldTextVariants())} style={{ marginBottom: 16 }} spacing="none">
                  {t('common:area')}
                </Text>
          ...
          <>
            {showInputs ? (
              <>
                ...
                <MovingDate defaultDate={date} setDatePicked={setDatePicked} positionBottom />
              </>
            ) : (
        ...
        {isAlwaysIncluded && (
          <>
            <div className={clsx(alwaysIncludedTextSectionVariants())}>
              <Check color="red" />
              <Text className={clsx(boldTextVariants())} spacing="none">
            ...
            <div className={clsx(alwaysIncludedTextSectionVariants({ noPadding: true }))}>
              <Check />
              <Text className={clsx(boldTextVariants())} spacing="none">
